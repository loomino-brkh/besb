...
class APIKeySerializer(serializers.ModelSerializer):
    class Meta:
        model = APIKey
        fields = ['id', 'name', 'key', 'created_at', 'expires_at', 'revoked', 'permission']
...
class CreateAPIKeySerializer(serializers.ModelSerializer):
    ...
    permission = serializers.ChoiceField(choices=PERMISSION_CHOICES, write_only=True, required=True)
    ...
    def validate_permission(self, value):
        ...
        if value not in allowed_permissions:
            raise serializers.ValidationError(f"Permission must be one of {allowed_permissions}.")
        return value
    ...
    def create(self, validated_data):
        try:
            ...
            raise serializers.ValidationError("An error occurred while creating the API key.")
